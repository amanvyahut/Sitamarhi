...
window.addEventListener('appinstalled', () => {
  ...
  showToast('✅ App installed! Dashboard ab home screen pe hai.', 'success');
});
function triggerPWAInstall() {
  if (_pwaPrompt) {
    _pwaPrompt.prompt();
    _pwaPrompt.userChoice.then(r => {
      if (r.outcome === 'accepted') showToast('✅ App install ho raha hai…', 'success');
      _pwaPrompt = null;
    });
  } else {
    // Fallback instructions
    showToast('📱 Browser menu → "Add to Home Screen" ya "Install App" select karo', 'info');
  }
}
...
function setupWizardDone() {
  ...
  loadChartJS();
  initFirebase();
  showToast('✅ Configuration save ho gayi! Dashboard ready hai.', 'success');
}
...
document.addEventListener('DOMContentLoaded',()=>{
  ...
  if(sessionStorage.getItem('tscric_setup_done')==='1'){
...
function updateDashboard(d){
  ...
  if(owmEl){owmEl.textContent=owmOK?'\ud83d\udfe2 OWM Live':'\ud83d\udd34 OWM Unavailable';owmEl.className='owm-status'; owmEl.style.color=owmOK?'var(--accent-green)':'var(--accent-red)';}
  ...
  if(owmOK){
    ...
    setEl('cmpTemp',el=>el.style.color=Math.abs(td)>3?'var(--orange)':'var(--accent-green)');
    setEl('cmpHum', el=>el.style.color=Math.abs(hd)>10?'var(--orange)':'var(--accent-green)');
  } else {
