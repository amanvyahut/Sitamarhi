/* ============================================================
   TSCRIC-LoRa Dashboard — Smart Irrigation Control System
   B.Tech Minor Project — Civil Engineering, OCT Bhopal, RGPV
   ============================================================ */

// ============================================================
// PWA INSTALL — Progressive Web App Shell
// ============================================================
let _pwaPrompt = null;
...
function wizardUseGPS() {
  ...
  if (!navigator.geolocation) {
    statusEl.textContent = '❌ GPS supported nahi hai.';
    return;
  }
  statusEl.textContent = '⏳ Location le raha hai...';
  navigator.geolocation.getCurrentPosition(
    function(pos) {
      const lat = pos.coords.latitude.toFixed(5);
      const lon = pos.coords.longitude.toFixed(5);
      statusEl.textContent = '✅ GPS mil gayi: ' + lat + '°N, ' + lon + '°E';
      localStorage.setItem('tscric_gps_lat', lat);
      localStorage.setItem('tscric_gps_lon', lon);
      localStorage.setItem('tscric_use_gps', '1');
    },
    function(err) {
      statusEl.textContent = '❌ GPS nahi mili. Dobara try karein.';
      localStorage.removeItem('tscric_use_gps');
    },
    { enableHighAccuracy: true, timeout: 15000 }
  );
}

// Auto GPS — silently get location on first open
function autoGetGPS() {
  ...
  navigator.geolocation.getCurrentPosition(
    function(pos) {
      ...
      localStorage.setItem('tscric_use_gps', '1');
      const statusEl = document.getElementById('wizardGpsStatus');
      if(statusEl) statusEl.textContent = '✅ GPS mil gayi: ' + lat + '°N, ' + lon + '°E';
      // If already on dashboard, fetch weather now
      if(localStorage.getItem('tscric_setup_done')==='1'){
        fetchOWMByCoords(parseFloat(lat), parseFloat(lon));
      }
    },
    function(err) { /* silent fail */ },
    { enableHighAccuracy: true, timeout: 15000 }
  );
}
...
function configUseGPS() {
  const statusEl = document.getElementById('configGpsStatus');
  if (!navigator.geolocation) {
    if(statusEl) statusEl.textContent = '❌ GPS supported nahi hai.';
    return;
  }
  if(statusEl) statusEl.textContent = '⏳ Location le raha hai...';
  navigator.geolocation.getCurrentPosition(
    function(pos) {
      const lat = pos.coords.latitude.toFixed(5);
      const lon = pos.coords.longitude.toFixed(5);
      if(statusEl) statusEl.textContent = '✅ GPS: ' + lat + '°N, ' + lon + '°E';
      localStorage.setItem('tscric_gps_lat', lat);
      localStorage.setItem('tscric_gps_lon', lon);
      localStorage.setItem('tscric_use_gps', '1');
      // Fetch weather with real GPS coords
      fetchOWMByCoords(parseFloat(lat), parseFloat(lon));
    },
    function(err) {
...
// ============================================================
// FIX (rainfall pipeline audit): shared rain-probability + current-rain
// helpers used by BOTH fetch paths so the logic can't drift apart again.
//
// Bug fixed: rain probability used to come ONLY from forecast POP of the
// *next* 3h bucket (f.list[0].pop). That value can legitimately be 0
// while it is raining right now, because "probability of precipitation
// in the next slot" and "is it raining right now" are different things.
// Per the required priority order:
//   1) actual rain sensor (tipping bucket) — handled in the .ino / dashboard priority chain
//   2) current OWM weather condition / rain field (IS it raining now)
//   3) weather condition code (Rain/Drizzle/Thunderstorm group)
//   4) forecast POP (fallback for "is it likely soon")
// If current rain is non-zero or the condition group says it's actively
// raining, probability is floored at 90% — it must never read 0% while
// raining.
// ============================================================
function owmConditionIsRaining(weatherArr){
  if(!weatherArr || !weatherArr.length) return false;
  const id = weatherArr[0].id;
  // OWM group codes: 2xx Thunderstorm, 3xx Drizzle, 5xx Rain.
  // FIX: the old range check `id>=200 && id<600` also matched Group 6xx
  // (Snow, codes 600-622) since it only bounded the upper end at 600 —
  // meaning snow conditions (which do exist in OWM's schema even if rare
  // for this project's location) were being misreported as rain. Now
  // checks each real rain-family group explicitly.
  return (id>=200 && id<300) || (id>=300 && id<400) || (id>=500 && id<600);
}

function computeOWMRainProbability(currentJson, forecastJson, currentRainMM){
  // Priority 2: currently raining (measured mm this hour) → definite rain
  if(currentRainMM>0) return 100;

  // Priority 4 data gathered first so it can also be used to refine the
  // condition-based estimate below, not just as a last-resort fallback.
  let pop = 0;
  if(forecastJson && forecastJson.list && forecastJson.list.length){
    const nextSlot = forecastJson.list[0];
    if(nextSlot.pop!==undefined) pop = nextSlot.pop*100;
  }

  // Priority 3: current condition code says rain/drizzle/thunderstorm.
  // FIX: previously this always returned a flat 90% regardless of
  // whether OWM reported a light drizzle or a heavy thunderstorm, which
  // made the number feel arbitrary/inaccurate and disconnected from the
  // forecast's own POP value. Now it's graduated by severity, and never
  // drops below the forecast's own POP for that slot (since the
  // condition is actively confirming rain right now, the number should
  // never read LOWER than what the forecast alone already suggested).
  if(currentJson && owmConditionIsRaining(currentJson.weather)){
    const id = currentJson.weather[0].id;
    let base;
    if(id>=200 && id<300) base=95;       // thunderstorm — high confidence
    else if(id>=500 && id<505) base=85;  // light/moderate/heavy rain
    else if(id>=500 && id<600) base=90;  // very heavy/extreme/freezing/shower rain
    else base=75;                        // drizzle (3xx) — lighter confidence
    return Math.round(Math.max(base,pop));
  }

  // Priority 4: forecast POP for the NEAR TERM only (next 3-hour slot).
  // "Rain Probability" here means "is it about to rain", not "could it
  // rain at some point today", so only the immediate next slot is used —
  // not a Math.max() across the whole fetched window, which would keep
  // the number pinned to whatever the worst slot in the next 24h is.
  return Math.round(pop);
}

// Fetch weather using exact GPS coordinates
function fetchOWMByCoords(lat, lon) {
  const wUrl = 'https://api.openweathermap.org/data/2.5/weather?lat='+lat+'&lon='+lon+'&appid='+OWM_DIRECT_KEY+'&units=metric';
  const fUrl = 'https://api.openweathermap.org/data/2.5/forecast?lat='+lat+'&lon='+lon+'&appid='+OWM_DIRECT_KEY+'&units=metric&cnt=8';
  fetch(wUrl).then(r=>r.json()).then(j=>{
    const owmRainNow = j.rain&&j.rain['1h']?j.rain['1h']:(j.rain&&j.rain['3h']?j.rain['3h']/3:0);
    if(owmRainNow>0) accumulateOWMRainfall(owmRainNow);
    // Get rain probability from forecast (with current-condition priority)
    fetch(fUrl).then(r=>r.json()).then(f=>{
      const rainProb = computeOWMRainProbability(j, f, owmRainNow);
      owmDirectData={owm_temp:j.main?j.main.temp:null,owm_humidity:j.main?j.main.humidity:null,owm_pressure:j.main?j.main.pressure:null,owm_rain_mm:owmRainNow,owm_rain_accum:getOWMAccumulated(),owm_rain_prob:rainProb,owm_condition:(j.weather&&j.weather[0]?j.weather[0].main:null),owm_valid:true};
      setText('weatherLocBanner', '📍 GPS Location');
      setText('weatherLocationInfo', '📍 Lat: '+lat+'°N  |  Lon: '+lon+'°E (GPS)');
      // Update OWM display directly
      const owmTempStr = owmDirectData.owm_temp !== null ? owmDirectData.owm_temp.toFixed(1)+' °C' : '--';
      const owmHumStr  = owmDirectData.owm_humidity !== null ? owmDirectData.owm_humidity.toFixed(0)+' %' : '--';
      const owmPresStr = owmDirectData.owm_pressure !== null ? owmDirectData.owm_pressure.toFixed(1)+' hPa' : '--';
      const owmRainStr = owmDirectData.owm_rain_mm !== null ? owmDirectData.owm_rain_mm.toFixed(2)+' mm' : '--';
      const owmRainProbStr = rainProb+'%';
      ...
      setText('owmRainProb', owmRainProbStr); setText('owmRainProb_s', owmRainProbStr);
      setText('owmStatus', '🟢 OWM Live'); setText('owmStatus_s', '🟢 OWM Live');
      // FIX: merge into dashboard so rainProb/rain widgets driven by
      // Firebase `d` actually see live OWM data instead of staying stuck
      // at whatever the last Firebase push contained (or 0).
      refreshDashboardWithOWM();
    }).catch(e=>{
      const rainProb = computeOWMRainProbability(j, null, owmRainNow);
      owmDirectData={owm_temp:j.main?j.main.temp:null,owm_humidity:j.main?j.main.humidity:null,owm_pressure:j.main?j.main.pressure:null,owm_rain_mm:owmRainNow,owm_rain_accum:getOWMAccumulated(),owm_rain_prob:rainProb,owm_condition:(j.weather&&j.weather[0]?j.weather[0].main:null),owm_valid:true};
      setText('weatherLocBanner', '📍 GPS Location');
      const owmTempStr = owmDirectData.owm_temp !== null ? owmDirectData.owm_temp.toFixed(1)+' °C' : '--';
      setText('owmTemp', owmTempStr); setText('owmTemp_s', owmTempStr);
  ...
  }).catch(e=>console.warn('GPS OWM fetch failed',e));
}
function setupWizardDone() {
  ...
  if (locEl) {
    ...
    // Only fetch city weather if GPS not used
    if (localStorage.getItem('tscric_use_gps') !== '1') {
      onWeatherLocationChange(loc);
    }
  }
  // If GPS was used, fetch weather by GPS coords immediately
  if (localStorage.getItem('tscric_use_gps') === '1') {
    const gLat = parseFloat(localStorage.getItem('tscric_gps_lat'));
    const gLon = parseFloat(localStorage.getItem('tscric_gps_lon'));
    if (!isNaN(gLat) && !isNaN(gLon)) {
      setTimeout(function(){ fetchOWMByCoords(gLat, gLon); }, 500);
      setTimeout(function(){ fetchOWMByCoords(gLat, gLon); }, 3000);
    }
  }
  const sw = document.getElementById('setupWizard');
...
// FIX (rainfall pipeline audit): owmDirectData now always has a consistent
// shape from the start (was `null`, causing scattered null-checks and
// silent failures anywhere code touched owmDirectData.owm_* before the
// first fetch resolved).
let owmDirectData = {owm_temp:null,owm_humidity:null,owm_pressure:null,owm_rain_mm:0,owm_rain_accum:0,owm_rain_prob:0,owm_condition:null,owm_valid:false};
...
document.addEventListener('DOMContentLoaded',()=>{
  ...
  // Auto GPS silently on every load
  autoGetGPS();

  if(localStorage.getItem('tscric_setup_done')==='1'){
    loadChartJS(); initFirebase();
    // Fetch weather with GPS after 3s
    setTimeout(function(){
      const gLat=parseFloat(localStorage.getItem('tscric_gps_lat'));
      const gLon=parseFloat(localStorage.getItem('tscric_gps_lon'));
      if(!isNaN(gLat)&&!isNaN(gLon)){
        fetchOWMByCoords(gLat,gLon);
      }
    },3000);
  } else {
    const sw=document.getElementById('setupWizard');
...
function updateDashboard(d){
  ...
  // FIX (rainfall pipeline audit): rainProb now computed via the shared
  // effectiveRainProb() helper (priority: tipping bucket firmware value
  // when hardware present → OWM current/forecast probability otherwise),
  // reused everywhere in this function (rainVal/rainKpi KPI tiles,
  // rainProbVal bar, rainCategory label) and by every other consumer
  // (explainability engine, AI chat, decision text) so they can never
  // disagree with each other again.
  const rainProb=effectiveRainProb(d);
  setText('rainVal',rainProb.toFixed(0));
  setText('rainKpi',rainProb.toFixed(0));
  ...
  setText('balanceVal',  balance.toFixed(1)+' L'); setText('totalFlowVal',totalInput.toFixed(1)+' L');
  setText('rainfallContrib',rainCtrib.toFixed(1)+' L');
  const pct=required>0?Math.min((totalInput/required)*100,100):0;
  ...
  setText('budgetRemPct',remPct.toFixed(1)+'% remaining');
  const eff=required>0?Math.min((totalInput/required)*100,100):0;
  setText('irrigEfficiency',eff.toFixed(1)+'%');

  // Civil Engineering parameter fields
  const crop_ce = CROP_DATA[localConfig.crop||0];
  const area_ce = fv(d.plotArea_m2)||localConfig.plotArea_m2||6;
  const eto_ce = fv(d.eto);
  // Duty of water D = A/Q where Q is flow in cumec, A is area in ha
  const flowRate_ce = fv(d.flowRate); // L/min
  const flowCumec = flowRate_ce / 1000 / 60; // convert L/min to m3/s = cumec
  const areaHa = area_ce / 10000;
  const duty_ce = flowCumec > 0 ? (areaHa / flowCumec).toFixed(1) : '--';
  setText('ce_delta', (crop_ce ? crop_ce.delta : '--')+' mm');
  setText('ce_area', area_ce.toFixed(1)+' m²');
  setText('ce_need', required.toFixed(0)+' L');
  setText('ce_eto_today', eto_ce > 0 ? eto_ce.toFixed(2)+' mm/day' : '--');
  setText('ce_duty', duty_ce !== '--' ? duty_ce+' ha/cumec' : '--');

  // Water balance equation components (ΔS = I + Pe − ETc − DP − R)
  const etcL = eto_ce > 0 ? (eto_ce * area_ce * 0.001).toFixed(1)+' L/day' : '--';
  setText('wb_applied', applied.toFixed(1)+' L');
  setText('wb_rain',    rainCtrib.toFixed(1)+' L');
  setText('wb_etc',     etcL);
  ...
  // FIX: clearer wording so it's unambiguous which number is "raining
  // right now" (live, resets between OWM's hourly readings) vs "total so
  // far today" (accumulated, only grows). Previously both were squeezed
  // into one string that could read oddly when only one was non-zero.
  if(owmMM>0){
    setText('owmRainfall', '🌧️ Live: '+owmMM.toFixed(2)+' mm/h  |  Today: '+owmAccumMM.toFixed(2)+' mm');
  } else if(owmAccumMM>0){
    setText('owmRainfall', '☁️ Not raining now  |  Today: '+owmAccumMM.toFixed(2)+' mm');
  } else {
    setText('owmRainfall', owmOK ? '☀️ No rain detected' : 'No data');
  }
  ...
  // rainProb was already computed once, above (priority chain: tipping
  // bucket firmware value → OWM current/forecast probability). Reused here.
  ...
  // FIX: previous labels ("Rain Expected" at 70%+) read as near-certain,
  // but a 70% forecast probability genuinely means it can still not
  // rain roughly 3 times out of 10 — that's how probability forecasts
  // work, not a bug in the number itself. Wording now stays honest about
  // uncertainty at every band instead of implying certainty too early.
  setText('rainCategory',rainProb<20?'\u2600\ufe0f Clear':rainProb<40?'\u26c5 Slight Chance':rainProb<70?'\ud83c\udf26\ufe0f Likely':rainProb<90?'\ud83c\udf27\ufe0f High Chance':'\ud83c\udf27\ufe0f Very Likely');
  ...
  setEl('aiRingFill',el=>{
    const maxScore=100, pct=Math.min(1, score/maxScore);
...
// ============================================================
// FIX (rainfall pipeline audit): single shared source of truth for
// "effective rain probability" — used by updateDashboard, the
// explainability engine, the AI chat, and the decision-reasoning text.
// Before this fix each of those places re-read d.rainProb directly and
// trusted a firmware 0 at face value, even when rainSensorPresent is
// false (no tipping bucket wired) — meaning "irrigation suppressed
// because of rain" explanations, AI-score rain-penalty text, and rain
// category badges could ALL disagree with what OWM was actually
// reporting, and would never show real rain probability without
// hardware. Priority: 1) tipping bucket firmware value (when hardware
// present) 2) OWM current/forecast probability.
// ============================================================
function effectiveRainProb(d){
  if(!d) return 0;
  const hasRainSensor=d.rainSensorPresent===true;
  const firmwareRainProb=fv(d.rainProb);
  const owmOK=d.owm_valid===true;
  const owmRainProb=owmOK?fv(d.owm_rain_prob):0;
  return hasRainSensor?firmwareRainProb:(firmwareRainProb>0?firmwareRainProb:owmRainProb);
}
...
function accumulateOWMRainfall(newMM){
  ...
  // FIX (rainfall pipeline audit): the old guard `newMM!==acc.lastVal`
  // permanently blocked accumulation once OWM reported the same steady
  // rain rate (e.g. 2.5 mm/h) on two consecutive fetches — a very common
  // case during continuous rain — because lastVal only resets at the
  // next calendar day. That silently dropped real rainfall for the rest
  // of the rain event, which is exactly the "rainfall mm not showing"
  // symptom. The de-dupe should only prevent counting the SAME OWM
  // "current hour" reading twice inside the same ~10-minute fetch cycle,
  // not block a legitimately repeated rain rate on the next cycle.
  const now=Date.now();
  const minGapMs=8*60*1000; // slightly less than the 10-min fetch interval
  if((now-acc.lastFetch)>minGapMs){
...
async function fetchOWMDirect(){
  ...
  try{
    ...
    // FIX: use shared helper (current condition + current rain amount
    // take priority over forecast POP) instead of ONLY reading forecast
    // POP, which could read 0% while it is raining right now.
    const rainProb=computeOWMRainProbability(j,jf,owmRainNow);
    ...
    // Note: 'owmRainfall' (the dedicated rainfall card) is set by
    // refreshDashboardWithOWM() → updateDashboard() below, using the
    // clearer "Live: X mm/h | Today: Y mm" format — not duplicated here
    // to avoid a flash of the shorter, less clear format first.
    ...
    // FIX: previously this block only ran `if(!lastData)` — meaning once a
    // single Firebase push had ever arrived, OWM's rain probability would
    // NEVER update the visible rain widgets again (they'd be frozen at
    // whatever the firmware last sent, which is 0 without a tipping
    // bucket). Now OWM always merges into the live dashboard via
    // refreshDashboardWithOWM(), with priority handled centrally there.
    refreshDashboardWithOWM();
...
// ============================================================
// FIX (rainfall pipeline audit): the missing bridge between OWM data
// (owmDirectData, fetched on its own timer) and the dashboard render
// path (updateDashboard(d), driven by Firebase pushes).
//
// Before this fix, updateDashboard() read d.owm_valid / d.owm_rain_mm /
// d.owm_temp directly off the raw Firebase snapshot — but the firmware
// NEVER writes those keys (see .ino Firebase schema), so d.owm_valid was
// always undefined, owmOK was always false, and every OWM-dependent
// widget (rain probability merge, temp/humidity comparison, OWM rainfall
// used in the analytics chart) silently never activated. This function
// merges the live owmDirectData into a shallow copy of the last known
// Firebase snapshot and re-renders, so OWM data actually reaches the UI
// and the chart buffers on its own 10-minute cycle, not just whenever a
// Firebase push happens to also arrive.
// ============================================================
function refreshDashboardWithOWM(){
  // Guard: don't run a full dashboard render off OWM data alone before any
  // real Firebase sensor data has ever arrived — that would push fake 0
  // readings for soil moisture / flow / AI score into the analytics chart
  // buffers, polluting them with zeros that never actually happened.
  // OWM-only widgets (temp/rain comparison text) already update directly
  // inside fetchOWMDirect()/fetchOWMByCoords(); this bridge only needs to
  // run once real sensor data exists to merge with.
  if(!lastData) return;
  const base = lastData;
  const merged = Object.assign({}, base, {
    owm_valid: !!(owmDirectData && owmDirectData.owm_valid),
    owm_temp: owmDirectData ? owmDirectData.owm_temp : null,
    owm_humidity: owmDirectData ? owmDirectData.owm_humidity : null,
    owm_pressure: owmDirectData ? owmDirectData.owm_pressure : null,
    owm_rain_mm: owmDirectData ? owmDirectData.owm_rain_mm : 0,
    owm_rain_accum: owmDirectData ? owmDirectData.owm_rain_accum : 0,
    owm_rain_prob: owmDirectData ? owmDirectData.owm_rain_prob : 0,
    owm_condition: owmDirectData ? owmDirectData.owm_condition : null
  });
...
function runAlertEngine(d,owmValid){
  ...
  if(d.offlineMode&&fv(d.offlineLogCount)>=40)
    addAlert('warning','\ud83d\udce1 EEPROM Near Full',fv(d.offlineLogCount)+' offline events pending. Buffer approaching 20-event limit.',true,'alert_eeprom');

  if(aiS>100) addAlert('warning','\ud83e\udd16 High AI Score','AI Score '+aiS.toFixed(1)+'/100 \u2014 extreme irrigation urgency.',true,'alert_hiai');
...
// ============================================================
// RULE-BASED DECISION EXPLAINABILITY ENGINE
// ============================================================
function updateExplainabilityEngine(d,owmValid){
  ...
  const aiScore=fv(d.aiScore),rainProb=effectiveRainProb(d),eto=fv(d.eto),flow=fv(d.flowRate),pumpOn=d.pump||false;
  ...
  // AI score
  if(aiScore>=65) cards.push({type:'warn',icon:'\ud83e\udd16',title:`AI Score ${aiScore.toFixed(1)}/100 \u2014 Irrigation Triggered`,text:`Score crossed 65-point threshold. Integrates CSMI (${csmi.toFixed(1)}%), moisture velocity (SMV), temporal pattern (TPR), and ETo demand. All conditions satisfied.`,badge:'TRIGGERED',badgeType:'warn'});
  else cards.push({type:'info',icon:'\ud83e\udd16',title:`AI Score ${aiScore.toFixed(1)}/100 \u2014 Monitoring`,text:`Score is ${(65-aiScore).toFixed(0)} points below trigger. System monitoring depletion rate (SMV) and temporal pattern (TPR: ${fv(d.tprScore).toFixed(3)}) until threshold crossed.`,badge:'MONITORING',badgeType:'info'});
  ...
  if(decisionText){
    ...
    else dec=`<strong style="color:var(--accent-green)">MONITORING \u2014 OPTIMAL:</strong> CSMI ${csmi.toFixed(1)}% in optimal range for ${CROP_DATA[localConfig.crop].name}. AI Score ${aiScore.toFixed(1)}/100. Standard monitoring cycle active (10s interval).`;
...
function updateTippingBucketPanel(d){
  ...
  // FIX (rainfall pipeline audit): use the same priority chain as the
  // rest of the dashboard (tipping bucket → OWM accumulated) instead of
  // `fv(d.owm_rain_mm)||tipMM`, which (a) relied on a field that never
  // reached `d` before the updateDashboard fix above, and (b) used OWM's
  // instantaneous current-hour reading rather than the accumulated total,
  // understating rainfall between fetch cycles.
  const priorityRain=getPriorityRainfall(tipMM);
  const rainMM=priorityRain.mm;
  // Accumulator should track whichever source is authoritative right now,
  // not just the tipping bucket — otherwise, with no tipping bucket
  // connected, tipAccum_mm stays at 0 forever even during real rain.
  if(rainMM>tipAccum_mm)tipAccum_mm=rainMM;
...
function checkRainfallIntelligence(){
  if(!lastData)return;
  // FIX (rainfall pipeline audit): previously only checked
  // lastData.tipBucket_mm, so with no tipping bucket connected this
  // alert could never fire even during a genuine heavy-rain event picked
  // up by OWM. Now uses the same tipping-bucket → OWM priority chain.
  const tipMM=fv(lastData.tipBucket_mm);
  const priorityRain=getPriorityRainfall(tipMM);
  if(priorityRain.mm>20)addAlert('info','\ud83c\udf27\ufe0f Significant Rainfall',priorityRain.src+' recorded '+priorityRain.mm.toFixed(1)+' mm. Irrigation suppressed. Water budget updated.',true,'alert_rain_event');
}
...
function appendChartData(d,owmOK){
  ...
  // FIX (rainfall pipeline audit): this is the root cause of "rainfall mm
  // not showing in the analytics graph". Two bugs stacked here:
  //  1) owmOK was always false because d.owm_valid never existed on the
  //     raw Firebase snapshot (see updateDashboard / refreshDashboardWithOWM
  //     fix above) — so the OWM fallback term was permanently dead code.
  //  2) Even with owmOK fixed, without a tipping bucket connected
  //     tipBucket_mm is always 0, so the chart needs the OWM value to be
  //     the PRIMARY line whenever no real rain sensor is present — using
  //     getPriorityRainfall() (tipping bucket → OWM accumulated) keeps
  //     this consistent with the rest of the dashboard's priority logic
  //     instead of a one-off `||` fallback that only looked at the
  //     instantaneous OWM reading (which resets to 0 between rain hours).
  ...
  B.rainfall.push(priorityRain.mm);
...
function buildFarmContext(){
  ...
  return [
    ...
    `AI Score:${_fv(d.aiScore).toFixed(1)}/100 | SMV:${_fv(d.smv).toFixed(4)} | TPR:${_fv(d.tprScore).toFixed(3)}`,
    `Rain Prob:${effectiveRainProb(d).toFixed(0)}%`,
    rainSrc,
...
function getRainInfo(){
  // FIX (rainfall pipeline audit): delegates to the same shared
  // effectiveRainProb() helper used by updateDashboard so this can never
  // disagree with what's shown on the dashboard. lastData may not have
  // owm_* merged in yet if no OWM fetch has completed since the last
  // Firebase push, so we merge owmDirectData in locally here too.
  const d=lastData, owm=owmDirectData;
  const merged=d?Object.assign({},d,{
    owm_valid: !!(owm&&owm.owm_valid),
    owm_rain_prob: owm?owm.owm_rain_prob:0
  }):null;
  const prob=effectiveRainProb(merged);
  const tipMM=d?fv(d.tipBucket_mm):0;
  const priorityRain=getPriorityRainfall(tipMM);
  return{prob,mm:priorityRain.mm,src:priorityRain.src};
}
...
function ruleBasedResponse(question){
  ...
  if(has(['pump','motor','pump not','pump status','motor status','pump on','pump off','relay','pump chalao','pump band','motor kharab'])){
    ...
    const flow=_fv(d.flowRate), total=_fv(d.totalLitres);
    const whyOff=!pumpOn?(effectiveRainProb(d)>75?'\ud83c\udf27\ufe0f Rain probability >75% — auto-suppressed':csmi>65?'\ud83d\udca6 Soil moisture adequate — no trigger':score<65?'\ud83e\udd16 AI score '+score.toFixed(1)+'/65 — threshold not reached':d.pipelineFault?'\ud83d\udd34 Pipeline fault — auto-stopped':d.safeMode?'\u26a0\ufe0f Safe mode — sensors failed':'Unknown'):'N/A';
  ...
  if(has(['ai score','score kya','irrigation score','score explain','trigger','csmi score','smv','sma','tpr','score kam','score zyada','how does ai work','ai kaise kaam'])){
    ...
    const components=[
      ...
      `Rain Penalty: -${_fv(d.rainProb)>75?40:_fv(d.rainProb)>35?15:0} pts (firmware tipping-bucket value only — AI score runs on-device, no internet dependency)`
    ];
...
function irrigationMasterResponse(intent,d,_fv,owm){
  ...
  switch(intent){
    case 'irrig_when':{
      ...
      L.push('AI Score: '+_fv(d.aiScore).toFixed(1)+'/100 | Rain: '+ri.prob.toFixed(0)+'%\n');
