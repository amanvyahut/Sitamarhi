...
function computeOWMRainProbability(currentJson, forecastJson, currentRainMM){
  ...
  if(currentJson && owmConditionIsRaining(currentJson.weather)) return 90;
  // Priority 4: forecast POP — take the MAX across all returned slots,
  // not just the first slot, so a rain window a few hours out isn't missed.
  let pop = 0;
  if(forecastJson && forecastJson.list){
    forecastJson.list.forEach(slot=>{
      if(slot.pop!==undefined) pop = Math.max(pop, slot.pop*100);
    });
  }
