...
function launchSetupWizard() {
  ...
  if (sw) { sw.style.display = 'flex'; setTimeout(injectGPSButtonInWizard, 100); return; }
}
...
// ============================================================
// ============================================================
// GPS LOCATION — Exact field coordinates
// ============================================================
function saveGPSLocation(lat, lon){
  const d = {lat:parseFloat(lat.toFixed(5)), lon:parseFloat(lon.toFixed(5)), saved:Date.now()};
  localStorage.setItem('tscric_gps_loc', JSON.stringify(d));
  return d;
}
function getGPSLocation(){
  try{ return JSON.parse(localStorage.getItem('tscric_gps_loc')||'null'); }catch(e){ return null; }
}
function clearGPSLocation(){
  localStorage.removeItem('tscric_gps_loc');
  showToast('GPS location cleared. City dropdown will be used.','info');
}
...
function requestGPS(onSuccess){
  ...
  navigator.geolocation.getCurrentPosition(
    pos=>{
      ...
      showToast('✅ GPS saved: '+lat.toFixed(4)+'°N, '+lon.toFixed(4)+'°E','success');
      if(onSuccess) onSuccess(lat, lon);
      fetchOWMDirect(); // Refresh with new coords
    },
    err=>{
      const msg = err.code===1?'Location permission denied. Allow in browser settings.':
...
// Inject GPS button into setup wizard automatically
function injectGPSButtonInWizard(){
  const locRow = document.querySelector('#setupWizard .wizard-location-row, #setupWizard select[id="wizardLocation"]');
  if(!locRow) return;
  if(document.getElementById('wizardGPSBtn')) return; // Already injected

  const parent = locRow.parentElement || locRow.closest('.wizard-field') || locRow.parentNode;
  const gps = getGPSLocation();

  const div = document.createElement('div');
  div.style.cssText = 'margin-top:10px;display:flex;gap:8px;align-items:center;flex-wrap:wrap;';
  div.innerHTML = `
    <button id="wizardGPSBtn" onclick="requestGPS(function(){injectGPSButtonInWizard();})"
    ...
    ${gps ? `<span id="wizardGPSStatus" style="color:#00d4aa;font-size:12px;flex:2;">
      ✅ GPS saved: ${gps.lat}°N, ${gps.lon}°E
      <button onclick="clearGPSLocation();injectGPSButtonInWizard();"
        style="background:none;border:1px solid #f85149;color:#f85149;padding:2px 6px;
               border-radius:4px;cursor:pointer;margin-left:6px;font-size:11px;">Clear</button>
    </span>` :
    `<span id="wizardGPSStatus" style="color:#8b949e;font-size:12px;flex:2;">
      📡 Ya neeche se city choose karo
    </span>`}
  `;
  parent.insertBefore(div, locRow);
}

 (Priority 2 — used only if tipping bucket = 0)
// ============================================================
...
async function fetchOWMDirect(){
  ...
  // GPS override — use exact field location if saved
  const gpsLoc = getGPSLocation();
  const lat = gpsLoc ? gpsLoc.lat : loc.lat;
  const lon = gpsLoc ? gpsLoc.lon : loc.lon;
  const locLabel = gpsLoc ? '📍 GPS ('+lat+'°N, '+lon+'°E)' : loc.label;
  ...
  try{
    ...
    // Smart rain probability boost — OWM forecast is often late
    // If currently raining (rain.1h > 0) → definitely 95%+
    const wxId=j.weather&&j.weather[0]?j.weather[0].id:0;
    const wxMain=j.weather&&j.weather[0]?j.weather[0].main:'';
    const isRainingNow = owmRainNow>0 ||
                         wxMain==='Rain' || wxMain==='Drizzle' ||
                         wxMain==='Thunderstorm' ||
                         (wxId>=200&&wxId<600);
    if(isRainingNow){
      rainProb=Math.max(rainProb,95);
      // Also accumulate — it IS raining even if rain.1h missing
      if(owmRainNow<=0){
        // Estimate ~0.3mm if OWM doesn't report exact amount
        accumulateOWMRainfall(0.3);
      }
    }
